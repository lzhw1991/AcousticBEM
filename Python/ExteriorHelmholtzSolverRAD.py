...
class ExteriorHelmholtzSolverRAD(ExteriorHelmholtzSolver):

    def __init__(self, *args, **kwargs):
        super(ExteriorHelmholtzSolverRAD, self).__init__(*args, **kwargs)
    ...
    def elementArea(self, i):
        a = self.aVertex[self.aElement[i, 0]]
        b = self.aVertex[self.aElement[i, 1]]
        return np.pi * (a[0] + b[0]) * np.sqrt((a[0] - b[0])**2 + (a[1] - b[1])**2)
